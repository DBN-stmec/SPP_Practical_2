...
#define N 500
 ...
 /*for (int i = 0; i < subRows; i++) // for each process allocate rows of size N
  ...
  while (epsilon <= diff)
  {
    ...
    {
      MPI_Send(&u[subRows-1][0], N, MPI_DOUBLE, world_rank + 1, 0, MPI_COMM_WORLD);
    ...
    {                                                //receives first and last row from these processes as well
      MPI_Send(&u[0][0],N,MPI_DOUBLE, world_rank-1,0,MPI_COMM_WORLD);
      MPI_Send(&u[subRows-1][0],N,MPI_DOUBLE, world_rank+1,0,MPI_COMM_WORLD);

      MPI_Recv(&prev_row[0],N,MPI_DOUBLE,world_rank-1,MPI_ANY_TAG,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(&next_row[0],N,MPI_DOUBLE,world_rank+1,MPI_ANY_TAG,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
